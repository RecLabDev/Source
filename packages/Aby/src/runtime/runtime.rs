...
impl AbyRuntime {
    ...
    pub fn build(mut self) -> Self {
        self.broadcast_channel.get_or_insert_with(|| InMemoryBroadcastChannel::default());
        self // etc..
    }
}
...
impl AbyRuntime {
    ...
    pub async fn send_broadcast(&self) -> Result<bool, AbyRuntimeError> {
        let broadcast = self.broadcast_channel.as_ref().ok_or(AbyRuntimeError::Uninitialized)?;
